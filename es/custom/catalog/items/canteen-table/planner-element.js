...
var brown = new Three.MeshLambertMaterial({ color: 0xac6c25 });
...
function makeObjectMaxLOD() {
  ...
  //central pillar
  var centralPillar = new Three.BoxGeometry(1, 3, 1);
  var canteen_table = new Three.Mesh(centralPillar, grey);

  // plane top
  var planeTop = new Three.Mesh(new Three.BoxGeometry(10, 0.5, 10), brown);
  planeTop.position.set(0, 1.5, 0);
  canteen_table.add(planeTop);

  //plane nuts
  for (var dx = -0.25; dx <= 0.25; dx += 0.5) {
    for (var dz = -0.25; dz <= 0.25; dz += 0.5) {
  ...
  // central axis
  var axis1 = new Three.Mesh(new Three.BoxGeometry(1, 15, 1), grey);
  axis1.rotation.x = 0.5 * Math.PI;
  axis1.position.set(0, -1, 0);
  canteen_table.add(axis1);

  //axis
  var axis2 = new Three.Mesh(new Three.BoxGeometry(1, 10, 1), grey);
  axis2.rotation.z = 0.5 * Math.PI;
  axis2.position.set(0, -1, -7);
  canteen_table.add(axis2);

  //axis
  var axis3 = new Three.Mesh(new Three.BoxGeometry(1, 10, 1), grey);
  axis3.rotation.z = 0.5 * Math.PI;
  axis3.position.set(0, -1, 7);
  canteen_table.add(axis3);

  //legs
  for (var gx = -3; gx <= 3; gx += 6) {
    for (var gz = 7; gz >= -7; gz -= 14) {
      var leg = new Three.Mesh(new Three.BoxGeometry(1, 3, 1), grey);
  ...
  //legs base
  for (var fx = -3; fx <= 3; fx += 6) {
    for (var fz = 8; fz >= -8; fz -= 16) {
      var legBase = new Three.Mesh(new Three.BoxGeometry(3, 0.25, 1), grey);
      legBase.rotation.y = 0.5 * Math.PI;
      legBase.position.set(fx, -3.5, fz + 1);
      canteen_table.add(legBase);

      // base nuts
      var baseNut1 = new Three.Mesh(new Three.CylinderGeometry(0.1, 0.1, 0.3, 6), black);
      ...
      var baseNut2 = new Three.Mesh(new Three.CylinderGeometry(0.1, 0.1, 0.3, 6), black);
  ...
  // chair back
  for (var fsx = -3; fsx <= 3; fsx += 6) {
    for (var fsz = -9.24; fsz <= 9.24; fsz += 18.48) {
      var chairBack = new Three.Mesh(new Three.BoxGeometry(4, 0.5, 2.5), brown);
  ...
  for (var fbx = -4.5; fbx <= 4.5; fbx += 9) {
    for (var fbz = -9.45; fbz <= 9.45; fbz += 18.9) {
      // chair support bar
      var support1 = new Three.Mesh(new Three.BoxGeometry(0.25, 0.125, 4), grey);
      support1.rotation.x = 0.5 * Math.PI;
      support1.position.set(fbx, 1.6, fbz);
      canteen_table.add(support1);

      var support2 = new Three.Mesh(new Three.BoxGeometry(0.25, 0.125, 4), grey);
      support2.rotation.x = 0.5 * Math.PI;
      support2.position.set(fbx - 3, 1.6, fbz);
      canteen_table.add(support2);

      //nut back chair
      var nutBack1 = new Three.Mesh(new Three.CylinderGeometry(0.1, 0.1, 0.6, 6), black);
      nutBack1.rotation.x = 0.5 * Math.PI;
      nutBack1.position.set(fbx, 2.5, fbz - 0.2);
      canteen_table.add(nutBack1);

      var nutBack2 = new Three.Mesh(new Three.CylinderGeometry(0.1, 0.1, 0.6, 6), black);
  ...
  for (var fdx = -3; fdx <= 3; fdx += 6) {
    for (var fdz = -7; fdz <= 7; fdz += 14) {
      // seat chair
      var seat = new Three.Mesh(new Three.BoxGeometry(4, 0.5, 4), brown);
  ...
  for (var fbsx = -4.5; fbsx <= 4.5; fbsx += 9) {
    for (var fbsz = -9; fbsz <= 9; fbsz += 18) {
      // seat support
      var seatSupport1 = new Three.Mesh(new Three.BoxGeometry(0.25, 0.125, 1), grey);
      seatSupport1.position.set(fbsx, -0.35, fbsz);
      canteen_table.add(seatSupport1);

      var seatSupport2 = new Three.Mesh(new Three.BoxGeometry(0.25, 0.125, 1), grey);
      seatSupport2.position.set(fbsx - 3, -0.35, fbsz);
      canteen_table.add(seatSupport2);

      // seat nuts
      var seatNut1 = new Three.Mesh(new Three.CylinderGeometry(0.1, 0.1, 0.8, 6), black);
      seatNut1.position.set(fbsx, -0.35, fbsz - 2);
      canteen_table.add(seatNut1);

      var seatNut2 = new Three.Mesh(new Three.CylinderGeometry(0.1, 0.1, 0.8, 6), black);
...
export default {
  ...
  render3D: function render3D(element, layer, scene) {
    ...
    var canteen_table_MaxLOD = new Three.Object3D();
    canteen_table_MaxLOD.add(objectMaxLOD.clone());

    var valuePosition = new Three.Box3().setFromObject(canteen_table_MaxLOD);
    ...
    var canteen_table_MinLOD = new Three.Object3D();
