...
export default {
  ...
  render2D: function render2D(element, layer, scene) {
    ...
    var newWidth = convert(element.properties.get('width').get('length')).from(element.properties.get('width').get('unit')).to(scene.unit);

    var newDepth = convert(element.properties.get('depth').get('length')).from(element.properties.get('depth').get('unit')).to(scene.unit);
    ...
    return React.createElement(
      ...
      React.createElement('line', { key: '2', x1: newWidth / 2, x2: newWidth / 2, y1: newDepth, y2: newDepth + 30, style: arrow_style }),
      React.createElement('line', { key: '3', x1: .35 * newWidth, x2: newWidth / 2, y1: newDepth + 15, y2: newDepth + 30, style: arrow_style }),
      React.createElement('line', { key: '4', x1: newWidth / 2, x2: .65 * newWidth, y1: newDepth + 30, y2: newDepth + 15, style: arrow_style }),
      React.createElement(
        'text',
        { key: '5', x: '0', y: '0',
  ...
  render3D: function render3D(element, layer, scene) {
    ...
    var newHeight = convert(element.properties.get('height').get('length')).from(element.properties.get('height').get('unit')).to(scene.unit);

    var newAltitude = convert(element.properties.get('altitude').get('length')).from(element.properties.get('altitude').get('unit')).to(scene.unit);
